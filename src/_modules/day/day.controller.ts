...
export class DayController {
  constructor(
    private readonly service: DayService,
  ...
  @Auth({ prefix })
  async create(@Res() res: Response, @Body() body: CreateDayDTO) {
  ...
  @ApiQuery({ type: PartialType(FilterDayDTO) })
  async findAll(
    @Res() res: Response,
    @Filter({ dto: FilterDayDTO }) filters: FilterDayDTO,
  ) {
  ...
  async delete(@Res() res: Response, @Param('id') id: Id) {
    ...
    return this.response.success(res, 'delete day successfully');
  }
}
